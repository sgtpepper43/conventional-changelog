...
function parseRawCommit(raw, options) {
  ...
  msg.subject = msg.subject.replace(/\s*(?:Closes|Fixes|Resolves)\s(#|([A-Z]+-))(\d+)/ig, function(_, p1,p2,p3) {
    if (p2) {
      msg.closes.push(p2+p3);
    }
    else {
      msg.closes.push(parseInt(p3,10));
    }
    return '';
  });

  lines.forEach(function(line) {
    match = line.match(/(?:Closes|Fixes|Resolves)\s((?:((#|([A-Z]+-))\d+)(?:\,\s)?)+)/ig);

    match && match.forEach(function(m) {
      m && m.split(',').forEach(function(i) {
        var issue = i.match(/[A-Z]+-(\d+)/ig);
        if (issue) {
          issue && msg.closes.push((issue[0]));
        }
        else {
          issue = i.match(/\d+/);
